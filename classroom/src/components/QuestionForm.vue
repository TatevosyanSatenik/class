...
<template>
  <div class="question-form">
    <h2>{{ question ? 'Edit Question' : 'Հարցումների կազմում' }}</h2>
    
    <div class="form-group">
      <label for="content">Հարցում:</label>
      <textarea
        id="content"
        v-model="content"
        rows="3"
        placeholder="Ավելացրու քո հարցումը այստեղ..."
        required
      ></textarea>
    </div>

    <div class="form-group">
      <label for="type">Հարցման տիպը:</label>
      <select id="type" v-model="type">
        <option value="text">Տեքստային</option>
		<!-- <option value="text">Թեստային</option> -->
    ...
    <div v-if="type === 'quiz'" class="answers-section">
      ...
      <div v-for="answer in answers" :key="answer.id" class="answer-group">
        ...
        <input
          ...
          :placeholder="'պատասխան' + answer.id"
          required
        >
      </div>
    </div>
    ...
    <div class="form-actions">
      <button class="submit-btn" @click="handleSubmit">
        {{ question ? 'Update Question' : 'Ավելացնել' }}
      </button>
